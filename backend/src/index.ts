...
type Player = {
  id: string;
  username: string;
  user_id?: string; // Optional, da nicht jeder Spieler ein registrierter Benutzer ist
  health: number;
  isAlive: boolean;
  skin: {
    ball: string;
...
app.post("/lobby/join", async (req: Request, res: Response) => {
  try {
...
io.on("connection", (socket) => {
  ...
  socket.on("join", async (data) => {
    ...
    console.log(
      "Aktualisierte connectedPlayers:",
      Object.entries(connectedPlayers).map(([id, player]) => ({
